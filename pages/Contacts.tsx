...
import { Plus, Search, Edit, Phone, Mail, Building, MapPin, X, User, MessageCircle } from 'lucide-react';
...
export const Contacts: React.FC = () => {
  ...
  useEffect(() => {
    if (user) setContacts(StorageService.getContacts(user.id));
  }, [user, isModalOpen]);
  ...
  const getWhatsappLink = (phone: string) => {
      ...
      // Simple heuristic: if it's a mobile number without country code, add BR code 55
      // Assuming numbers with length 10 or 11 are local
                ...
                <div className="space-y-3 pt-4 border-t border-slate-50">
                    ...
                    {contact.address && (
                        <a 
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-start gap-3 text-sm text-slate-600 bg-slate-50/50 p-2 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-colors group/action cursor-pointer"
                            title="Ver no Google Maps"
                        >
                            <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-indigo-400 shadow-sm shrink-0 mt-0.5 group-hover/action:bg-blue-500 group-hover/action:text-white transition-all">
                                <MapPin size={14} />
                            </div>
                            <span className="text-xs leading-relaxed py-1 font-medium">{contact.address}</span>
                        </a>
                    )}
                </div>
            </div>
              ...
              <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden transform transition-all scale-100 border border-white/20">
                  ...
                  <div className="px-8 py-6 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100 flex justify-between items-center">
                      <div>
                          ...
                          <p className="text-slate-400 text-sm font-medium">Preencha os dados do cliente</p>
                      </div>
                      <button onClick={closeModal} className="text-slate-400 hover:text-red-500 hover:bg-red-50 p-2.5 rounded-full transition-colors">
                          <X size={24} />
                      </button>
                  </div>
                      ...
                      <div className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                              <div className="space-y-2">
                                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-4 flex items-center gap-1">
                                      Nome <span className="text-red-500 text-lg leading-none">*</span>
                                  </label>
                                  <div className="relative">
                                      ...
                                      <input 
                                          ...
                                          required 
                                          className="w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-full focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all font-medium text-slate-700"
                                          value={formData.name} 
                          ...
                          <div className="space-y-2">
                              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-4 flex items-center gap-2">
                                  Endereço Completo
                              </label>
                              <div className="relative">
                                  ...
                                  <textarea
                                      ...
                                      rows={2}
                                      className="w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-3xl focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all font-medium text-slate-700 resize-none leading-relaxed"
                      ...
                      <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                          ...
                          <div className="flex gap-3">
                              <button 
                                type="button" 
                                onClick={closeModal} 
                                className="px-6 py-3.5 text-slate-600 hover:bg-slate-100 rounded-full font-semibold transition-colors"
                              >
                                Cancelar
                              </button>
                              <button 
                                type="submit" 
                                className="px-8 py-3.5 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 font-bold shadow-lg shadow-indigo-200 hover:shadow-xl hover:scale-105 transition-all transform active:scale-95"
                              >
